...
if __name__ == '__main__':
    ...
    Button(root, text='Please Click to Choose Holdings File JPB to Import', command = openFile).pack(fill=X)
    mainloop()
...
secclass_country['Classifier Code'] = "Ev_Country_JPB(Holding)"
...
secclass_filename_country = "SECCLASS_Country_Holding_JPB_"+(time.strftime("%Y%m%d_%H%M%S")+".csv")
...
secclass_country_risk['Classifier Code'] = "Ev_Country_of_Risk_JPB"
...
secclass_filename_country_risk = "SECCLASS_Country_Risk_JPB_"+(time.strftime("%Y%m%d_%H%M%S")+".csv")
